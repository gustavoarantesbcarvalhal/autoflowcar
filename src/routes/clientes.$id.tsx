...
import { STATUSES, SOURCES, formatPriceBRL, whatsappLink, sourceLabel, statusLabel } from "@/lib/crm";
import { ArrowLeft, MessageCircle, Phone, Mail, MapPin, Trash2, CheckCircle2 } from "lucide-react";
...
async function fetchCustomer(id: string) {
  const [c, i, a] = await Promise.all([
    ...
    supabase.from("appointments").select("*").eq("customer_id", id).order("scheduled_at", { ascending: true }),
  ]);
  if (c.error) throw c.error;
  return { customer: c.data, interactions: i.data ?? [], appointments: a.data ?? [] };
}
...
function ClienteDetalhe() {
  ...
  const addInteraction = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("interactions").insert({ customer_id: id, type: type as never, content: note });
      if (error) throw error;
      await supabase.from("customers").update({ last_contact_at: new Date().toISOString() }).eq("id", id);
    },
    onSuccess: () => { setNote(""); qc.invalidateQueries({ queryKey: ["customer", id] }); toast.success("Registrado"); },
      ...
      <header className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
            ...
            {c.city && <span className="inline-flex items-center gap-1"><MapPin className="size-3.5" />{c.city}</span>}
            <span className="rounded bg-muted px-1.5 py-0.5 text-[10px] font-bold uppercase">{sourceLabel(c.source)}</span>
          </div>
        </div>
        <div className="flex gap-2">
          <a href={whatsappLink(c.whatsapp ?? c.phone, `Olá ${c.name}!`)} target="_blank" rel="noreferrer"
          ...
          <button onClick={() => { if (confirm("Excluir cliente?")) remove.mutate(); }}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          ...
          <section className="rounded-xl border border-border bg-card p-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <button onClick={() => addInteraction.mutate()} disabled={addInteraction.isPending}
          ...
          <section>
            ...
            {data.interactions.length === 0 ? (
              <p className="rounded-md border border-dashed border-border p-6 text-center text-sm text-muted-foreground">Nenhum registro ainda.</p>
            ) : (
                  ...
                  <li key={it.id} className="relative">
                    ...
                    <div className="rounded-lg border border-border bg-card p-3">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-primary">{it.type.replace("_", " ")}</span>
                        <span className="font-mono text-[10px] text-muted-foreground">
                          {new Date(it.created_at).toLocaleString("pt-BR")}
                        </span>
                      </div>
                      {it.content && <p className="mt-2 whitespace-pre-wrap text-sm">{it.content}</p>}
                    </div>
                  </li>
                ))}
              </ol>
        ...
        <aside className="space-y-4">
          <div className="rounded-xl border border-border bg-card p-4">
            ...
            <select value={c.status} onChange={(e) => updateCustomer.mutate({ status: e.target.value })}
              className="h-9 w-full rounded-md border border-border bg-background px-2 text-sm">
              {STATUSES.map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}
            </select>
            <label className="mt-3 block">
              <span className="text-[10px] font-bold uppercase text-muted-foreground">Próximo retorno</span>
              <input type="datetime-local"
                defaultValue={c.next_return_at ? new Date(c.next_return_at).toISOString().slice(0, 16) : ""}
                onChange={(e) => updateCustomer.mutate({ next_return_at: e.target.value ? new Date(e.target.value).toISOString() : null })}
                className="mt-1 h-9 w-full rounded-md border border-border bg-background px-2 text-sm" />
            </label>
            <button onClick={() => { updateCustomer.mutate({ last_contact_at: new Date().toISOString() }); toast.success("Contato registrado"); }}
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            ...
            <dl className="space-y-1 text-sm">
              <Row k="Marca" v={c.interest_brand} />
              <Row k="Modelo" v={c.interest_model} />
              <Row k="Ano" v={c.interest_year} />
              <Row k="Faixa" v={c.price_min || c.price_max ? `${formatPriceBRL(c.price_min)} – ${formatPriceBRL(c.price_max)}` : "—"} />
              <Row k="Origem" v={sourceLabel(c.source)} />
              <Row k="Status atual" v={statusLabel(c.status)} />
            </dl>
            {c.notes && <p className="mt-3 whitespace-pre-wrap rounded-md bg-muted p-2 text-xs">{c.notes}</p>}
          </div>
          ...
          {c.status === "perdido" && (
            <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-4">
              ...
              <textarea
                defaultValue={c.lost_reason ?? ""} rows={3}
